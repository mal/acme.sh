...
aws_s3_deploy() {
  ...
  _prefix="$(printf '/%s/' "$_prefix" | sed 's://\+:/:g')"

  if ! _aws s3 PUT "$_bucket" "$_prefix$_cdomain.cer" "$_region" <"$_cfullchain" >/dev/null; then
    _err "unable to deploy $_cdomain cert to s3://$_bucket$_prefix in $_region"
    _ret=2
  fi
  if ! _aws s3 PUT "$_bucket" "$_prefix$_cdomain.key" "$_region" <"$_ckey" >/dev/null; then
    _err "unable to deploy $_cdomain key to s3://$_bucket$_prefix in $_region"
    _ret=2
  fi
